...
import org.example.controller.DogCareController;
...
public class DogPage {
    ...
    private DogCareController dogCare;
    private ControlRewardModel controlReward;
    private DogLevelModel dogLevel;

    public DogPage() {
        this.controlReward = new ControlRewardModel();
        this.dogCare = new DogCareController(dogLevel, controlReward);
        this.dogLevel = new DogLevelModel();
        ...
        feedButton.addActionListener((ActionEvent e) -> {
            dogCare.careDog();  // DogCareController에 요청
            updateUI();
        });
        ...
        dogButton.addActionListener(e -> {
            dogCare.touchDog();  // DogCareController에 요청
            updateUI();
        });
    ...
    public void updateUI() {
        dogButton.setIcon(new ImageIcon(dogLevel.growUp(dogLevel.getLevel())));  // 레벨에 맞는 성장 단계의 강아지 이미지 아이콘 사용
        rewardLabel.setText("보상 : " + controlReward.getReward() + "개");  // 현재 보상을 반환
        levelLabel.setText("level " + dogLevel.getLevel());  // 현재 레벨을 반환
        closenessProgressBar.setValue(dogLevel.getCloseness());  // 현재 친밀도를 반환
    }
}